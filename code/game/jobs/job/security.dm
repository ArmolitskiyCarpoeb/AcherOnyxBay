/datum/job/hos
	...
	description = "Порядок. Безопасность. Защита. Ты здесь для подавления бунтов, для защиты экипажа от предателей корпорации (они среди нас) и для управления болванами - оперативниками."
	head_position = 1
	department = "Security"
...
/datum/job/hos/equip(mob/living/carbon/human/H)
	...
	if(.)
		...
		H.newgeneratestats(13,17,12,18,7,13,10,20)
		H.generate_skills()
...
/datum/job/officer
	...
	description = "Ты — инструмент, а не герой. Приказ слепо выполнен — вот и вся твоя доблесть. Хорошо подумай прежде чем рискнуть жизнью ради кого-то: на твоё место уже стоит в очереди следующий."
	department = "Security"
	department_flag = SEC
...
/datum/job/officer/equip(mob/living/carbon/human/H)
	...
	if(.)
		var/obj/item/implant/death_alarm/D = new()
		D.implant_in_mob(H, BP_HEAD)
		H.newgeneratestats(12,14,11,13,7,11,11,15)
		H.generate_skills()
