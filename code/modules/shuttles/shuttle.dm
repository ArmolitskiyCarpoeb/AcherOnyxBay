...
/datum/shuttle
	...
	var/ceiling_type = /turf/simulated/floor/reinforced // /turf/unsimulated/floor/shuttle_ceiling
...
/datum/shuttle/proc/short_jump(obj/effect/shuttle_landmark/destination)
	...
	if(sound_takeoff)
		playsound(current_location, sound_takeoff, 500, 35, 25)
			...
			while (world.time < arrive_time)
				if(!fwooshed && (arrive_time - world.time) < 100)
					...
					if(play_arrive_sound(destination))
						playsound(destination, sound_landing, 500, 35, 25)
				sleep(5)
...
/datum/shuttle/proc/play_arrive_sound(obj/effect/shuttle_landmark/destination)
	...
	for(var/mob/M in GLOB.player_list)
		...
		if(get_dist(destination, M) <= shuttle_size)
			M.playsound_local(M.loc, 'sound/effects/vessel_passby.ogg', 60, TRUE)
	return TRUE
