...
/mob/living/carbon/human/make_interaction()
	...
	var/obj/item/organ/external/temp = H.external_organs_by_name[BP_R_HAND]
	var/hashands = (temp && temp.is_usable())
	if (!hashands)
		temp = H.external_organs_by_name[BP_L_HAND]
		hashands = (temp && temp.is_usable())
	temp = P.external_organs_by_name[BP_R_HAND]
	var/hashands_p = (temp && temp.is_usable())
	if (!hashands_p)
		temp = P.external_organs_by_name[BP_L_HAND]
		hashands = (temp && temp.is_usable())
...
/mob/living/carbon/human/proc/do_fucking_animation(mob/living/carbon/human/P)
	...
	animate(src, pixel_x = pixel_x + pixel_x_diff, pixel_y = pixel_y + pixel_y_diff, time = 1)
	animate(pixel_x = initial(pixel_x), pixel_y = final_pixel_y, time = 1)

/mob/living/carbon/human/proc/has_bottom_underwear()
	for(var/obj/item/underwear/UW in worn_underwear)
		if(UW.underwear_slot & UNDERWEAR_SLOT_BOTTOM)
			return TRUE
	return FALSE
