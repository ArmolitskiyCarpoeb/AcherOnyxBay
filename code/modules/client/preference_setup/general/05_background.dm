...
/datum/category_item/player_setup_item/general/background/content(mob/user)
	...
	if(pref.home_system == "Acheron")
		. += "Мёртвая планета Ахерон, под землёй которой мы сейчас находимся. Стала таковой из-за Корпоративной Войны 59-69 годов. Ранее её называли Эдем из-за благоприятного климата.<br>"
