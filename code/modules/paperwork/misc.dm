...
/obj/item/paper/directive
	name = "Бланк описи погруженных ресурсов ОС \"ЧУЖБИНА\""
	desc = "Специальный бланк для оформления поставок. Должен быть заверен печатью квартирмейстера."
	var/is_copy = FALSE

/obj/item/paper/directive/Initialize()
	info = ""
	info += "\[br]\[center]<large><b>Отдел поставок ОС \"ЧУЖБИНА\"</b></large>\[/center]"
	info += "\[br]\[center]\[large] ОПР-ФЗД314С \[/large]\[/center]"
	info += "\[hr]"
	info += "\[br]\[large]Данным документом заверяется погрузка следующих ресурсов на челнок снабжения:\[/large] \[field]"
	info += "\[hr]"
	info += "\[br]\[small]Подпись составителя:\[/small]\[signfield]"
	info += "\[hr]"
	info += "\[br]Место для штампа"
	info += "\[br]\[date]"
	. = ..()
